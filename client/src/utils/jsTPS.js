...
export class UpdateRegion_Transaction extends jsTPS_Transaction {
    ...
    async undoTransaction() {
		let data;
        const { _id } = this.region.parentRegion;
        this.region.parentRegion = _id;
...
export class EditRegion_Transaction extends jsTPS_Transaction {
	constructor(_id, field, updated, prev, callback) {
		super();
		this._id = _id;
		this.field = field;
	...
	async doTransaction() {
		const { data } = await this.updateFunction({ 
				variables:{  _id: this._id, field: this.field, value: this.updated }
			});
		return data;
    }
